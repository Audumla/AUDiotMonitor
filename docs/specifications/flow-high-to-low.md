...
- Start in `foundation/overview.md` and `foundation/spec-100-component-architecture.md`.
- Define the problem, boundaries, and ownership.

## Phase 2: Contract

- Define expected behavior in `exporter/detailed-specification.md`.
...
- Capture implementation sequence in `exporter/implementation-plan-stages.md`.
- Capture release/deployment tradeoffs in `operations/optimization-review-2026-03.md`.

## Example (New Adapter Source Type)

1. Define source intent in `foundation/spec-100-component-architecture.md`.
2. Add data/endpoint contract requirements in `exporter/detailed-specification.md`.
3. Add adapter-specific rules in `adapters/`.
4. Add mapping rules and examples in `exporter/mapping-rule-spec.md`.
5. Add rollout steps in `exporter/implementation-plan-stages.md`.
